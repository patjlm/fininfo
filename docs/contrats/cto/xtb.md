---
nom: XTB
type: contrat-cto
slug: xtb
courtier: XTB
derniere-verification: 2026-05-17
---

# XTB — Compte-Titres

> Compte-titres proposé par [XTB](https://www.xtb.com/fr) (Pologne / succursale française). Courtier en ligne fondé en 2004, coté à la Bourse de Varsovie depuis 2016, avec plus de 2 millions de clients dans le monde. Voir aussi la fiche [CTO (enveloppe)](../../enveloppes/cto.md) pour les règles générales.

## Informations générales
<!-- Mis à jour : 2026-05-17 -->

- **Courtier** : XTB S.A. Succursale française — 20 Avenue André Prothin, 92400 Courbevoie — RCS Paris 522 758 689
- **Pays / régulation** : Pologne (KNF) — succursale française contrôlée par l'AMF (article L.621-9 du Code monétaire et financier) et l'ACPR, sous le N° 11533 LS
- **Domiciliation du compte** : France (pas de déclaration de compte étranger nécessaire)
- **Pays disponibles** : présent dans plus de 12 pays sur 3 continents
- **Dépôt initial minimum** : 1 EUR
- **CTO enfant** : non
- **Année de création** : 2004

## Frais
<!-- Mis à jour : 2026-05-17 -->

- **Frais — Actions françaises** :
  - Ordre de 10 EUR : 0 EUR
  - Ordre de 100 EUR : 0 EUR
  - Ordre de 500 EUR : 0 EUR
  - Ordre de 1 000 EUR : 0 EUR
- **Frais — Actions US** :
  - Ordre de 10 EUR : 0,05 EUR (frais de change 0,5 %)
  - Ordre de 100 EUR : 0,50 EUR (frais de change 0,5 %)
  - Ordre de 500 EUR : 2,50 EUR (frais de change 0,5 %)
  - Ordre de 1 000 EUR : 5 EUR (frais de change 0,5 %)
- **Frais — ETF européens** :
  - Ordre de 10 EUR : 0 EUR
  - Ordre de 100 EUR : 0 EUR
  - Ordre de 500 EUR : 0 EUR
  - Ordre de 1 000 EUR : 0 EUR
- **Frais de change** : 0,5 % (0,8 % les week-ends et jours fériés)
- **Frais divers** :
  - Frais d'inactivité : 10 EUR/mois après 12 mois sans transaction ET 90 jours sans dépôt
  - Frais de garde : 0,2 %/an (min. 10 EUR/mois) si portefeuille > 250 000 EUR (valeur quotidienne moyenne)
  - Retrait : gratuit
  - Dépôt par virement bancaire : gratuit
  - Au-delà de 100 000 EUR de volume mensuel : commission de 0,2 % (min. 10 EUR)
- **Exemples concrets (scénario mixte)** : 3 ordres actions US (100 EUR) + 3 ordres actions US (500 EUR) + 6 ordres ETF européens = frais de conversion uniquement : (300 + 1 500) x 0,5 % = 9 EUR ; ETF : 0 EUR ; total = 9 EUR
- **Grille tarifaire complète** : [XTB — Table des frais et commissions (PDF)](https://xtb.scdn5.secure.raxcdn.com/file/0103/39/Table_des_frais_et_commissions_783a56ebd4.pdf)
- **Note frais** : 4,5/5

## Produits et fonctionnalités
<!-- Mis à jour : 2026-05-17 -->

- **Actions** : + 6 800 sur 14 marchés (bourses américaines et européennes)
- **ETF** : + 2 000
- **Crypto** : non en direct — uniquement via CFD
- **Fractions d'actions/ETF** : oui — à partir de 10 EUR, droits fiduciaires sur fractions (pas de droit de vote sur les fractions)
- **Investissement programmé (DCA)** : oui — plans d'investissement (Investment Plans) disponibles, conçus pour l'investissement passif en ETF
- **Rémunération des liquidités** : oui — taux préférentiel les 90 premiers jours (2,30 % p.a. pour EUR au 2026-05-17), puis taux standard ; jusqu'à 100 000 EUR tous comptes confondus ; taux variable mis à jour chaque semaine ; intérêts calculés quotidiennement, versés mensuellement
- **Plateforme de trading** : xStation 5 (web et mobile) — outils d'analyse, cours en temps réel, contenus pédagogiques, webinaires
- **Note produits** : 2,5/5

## Propriété et transfert
<!-- Mis à jour : 2026-05-17 -->

- **Propriété des titres** : détention réelle pour les actions et ETF entiers (droits de propriété, dividendes) — conservation via BNP Paribas en France (compte en nom collectif) ; fractions d'actions : droits fiduciaires (pas de droit de vote)
- **Transfert de compte** : oui — transfert sortant à 25 EUR/ISIN (actions cotées en Espagne : 0,10 % min. 100 EUR/ISIN) ; transfert entrant gratuit
- **IFU fourni** : oui
- **Déclaration compte étranger** : non nécessaire (compte domicilié en France)
- **Protection des dépôts** : système d'indemnisation KDPW (Pologne) — 100 % jusqu'à 3 000 EUR + 90 % de l'excédent dans la limite de 22 000 EUR (max. 20 100 EUR par investisseur)
- **Note propriété** : 3,5/5

## Conditions et contraintes spécifiques
<!-- Mis à jour : 2026-05-17 -->

- **Types d'ordres** : marché, limite, stop — sur la plateforme xStation 5
- **Marchés accessibles** : 14 bourses (États-Unis, Allemagne, France, Royaume-Uni, Pologne, et autres bourses européennes)
- **Restrictions** : les CFD se trouvent dans le même compte que les actifs réels — attention à bien investir en actifs réels et non en CFD pour le long terme ; pas de SRD
- **Particularités fiscales** : compte français — IFU fourni, plus-values soumises au PFU (30 %) ou barème progressif ; XTB propose aussi un PEA séparément

## Expérience client
<!-- Mis à jour : 2026-05-17 -->

- **Avis clients (Trustpilot)** : 4/5 (2 300 avis)
- **Contact** : téléphone (01 53 89 63 30), email (commercial@xtb.fr), sur place à Paris (Courbevoie)
- **Test SAV** : quelques minutes d'attente (test sinvestir)
- **Note SAV** : Non communiqué

## Avis synthétique
<!-- Mis à jour : 2026-05-17 -->

- **Note globale** : 4,08/5
- **Points forts** :
  - 0 % de commission sur actions et ETF jusqu'à 100 000 EUR/mois
  - Courtier régulé en France (AMF/ACPR) avec IFU fourni — pas de déclaration de compte étranger
  - Rémunération des liquidités non investies
  - Plans d'investissement programmés (DCA) disponibles
  - Fractions d'actions et ETF à partir de 10 EUR
  - PEA également disponible chez le même courtier
  - Service client en français avec bureau à Paris
- **Points faibles** :
  - CFD et actifs réels dans le même compte — risque de confusion
  - Frais de change de 0,5 % sur les instruments en devises étrangères
  - Frais de garde de 0,2 %/an si portefeuille > 250 000 EUR
  - Frais d'inactivité (10 EUR/mois après 12 mois sans trade)
  - Protection des dépôts limitée (max. 20 100 EUR via KDPW)
  - Crypto uniquement via CFD (pas de détention directe)
- **Services complémentaires** : PEA, outils d'analyse de marché, contenus pédagogiques (cours et webinaires), news en temps réel

## Sources

- [S'investir — Comparatif Compte-Titres](https://simulateurs.sinvestir.fr/les-comparateurs/compte-titres)
- [XTB — Conditions et frais](https://www.xtb.com/int/account-and-fees)
- [XTB — Actions](https://www.xtb.com/int/stocks)
- [XTB — ETF](https://www.xtb.com/int/etfs)
- [XTB — Rémunération des liquidités](https://www.xtb.com/fr/interets)
- [XTB — Protection des fonds clients](https://www.xtb.com/fr/formation/protection-fonds-clients)
- [XTB — Régulation](https://www.xtb.com/fr/formation/xtb-regule)
- [XTB — Table des frais et commissions (PDF)](https://xtb.scdn5.secure.raxcdn.com/file/0103/39/Table_des_frais_et_commissions_783a56ebd4.pdf)
