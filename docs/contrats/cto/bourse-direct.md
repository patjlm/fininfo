---
nom: Bourse Direct
type: contrat-cto
slug: bourse-direct
courtier: Bourse Direct
derniere-verification: 2026-05-26
---

# Bourse Direct — Compte-Titres

> Compte-titres proposé par Bourse Direct (France).

## Informations générales
<!-- Mis à jour : 2026-05-17 -->

- **Courtier** : Bourse Direct
- **Pays / régulation** : France — Autorité des Marchés Financiers (AMF), Autorité de Contrôle Prudentiel et de Résolution (ACPR)
- **Domiciliation du compte** : France
- **Pays disponibles** : 1 (France uniquement)
- **Dépôt initial minimum** : 0 EUR (1 000 EUR recommandé)
- **CTO enfant** : non — CTO individuel ou joint uniquement (PEA Jeunes disponible séparément)
- **Année de création** : 1996 (30 ans d'existence)
- **Types de comptes** : individuel, joint (couples mariés/pacsés), indivision

## Frais
<!-- Mis à jour : 2026-05-17 -->

- **Frais — Actions françaises (Euronext Paris, Amsterdam, Bruxelles)** :
  - Ordre jusqu'à 500 EUR : 0,99 EUR
  - Ordre de 500 à 1 000 EUR : 1,90 EUR
  - Ordre de 1 000 à 2 000 EUR : 2,90 EUR
  - Ordre de 2 000 à 4 400 EUR : 3,80 EUR
  - Ordre supérieur à 4 400 EUR : 0,09 %
- **Frais — Actions US (NYSE / NASDAQ)** :
  - Ordre jusqu'à 10 000 EUR : 8,50 EUR
  - Ordre supérieur à 10 000 EUR : 0,09 %
- **Frais — ETF européens (Euronext)** :
  - Ordre jusqu'à 500 EUR : 0,99 EUR
  - Ordre de 500 à 1 000 EUR : 1,90 EUR
  - Ordre de 1 000 à 2 000 EUR : 2,90 EUR
  - Ordre de 2 000 à 4 400 EUR : 3,80 EUR
  - Ordre supérieur à 4 400 EUR : 0,09 %
- **Frais — Autres marchés étrangers** :
  - Londres (LSE) : 0,15 % minimum 15 EUR
  - Francfort (Xetra) : 0,15 % minimum 15 EUR
  - Madrid : 0,20 % minimum 18 EUR
  - Suisse (SIX / Virt-x) : 0,20 % minimum 18 EUR
  - Portugal (Euronext Lisbonne) : 0,20 % minimum 18 EUR
  - Autres marchés : 0,48 % minimum 41,90 EUR
- **Frais de change** : taux Bourse Direct + 0,08 % par opération
- **Frais divers** :
  - Droits de garde : 0,036 % / an (hors Euronext Paris, Bruxelles, Amsterdam)
  - Droits de garde Euronext : 0 EUR
  - Tenue de compte : 0 EUR
  - Ouverture / clôture : 0 EUR
  - Frais d'inactivité : 0 EUR
  - Souscription / rachat OPCVM : 0 EUR
  - Ordres par téléphone : majoration de 9,90 EUR
- **Offre spéciale ETF Amundi** : 0 EUR de frais de courtage sur le 1er ordre d'achat par mois sur une sélection d'ETF Amundi (ordres de 200 à 100 000 EUR, du 01/05/2026 au 30/04/2027)
- **Offre spéciale Morgan Stanley** : 0 EUR de frais sur warrants, certificats, turbos (ordres >= 500 EUR) ; 2,50 EUR pour ordres < 500 EUR
- **Exemples concrets (scénario mixte)** :
  - Actions US : 6 x 8,50 EUR = 51 EUR
  - Frais de conversion : 3 x 100 x 0,08 % + 3 x 500 x 0,08 % = 1,44 EUR
  - ETF : 6 x 0,99 EUR = 5,94 EUR
  - Total : 58,38 EUR
- **Grille tarifaire complète** : [Bourse Direct — Tarifs (PDF)](https://www.boursedirect.fr/pdf/tarifs_bd.pdf)
- **Note frais** : 2,5/5 *(source : sinvestir)*

## Produits et fonctionnalités
<!-- Mis à jour : 2026-05-17 -->

- **Actions** : + 10 000 disponibles (françaises, européennes, internationales)
- **ETF** : + 1 500 disponibles
- **OPCVM** : + 6 000 fonds accessibles (0 EUR de frais de souscription/rachat)
- **Produits dérivés** : warrants, certificats, turbos, futures (CAC 40, Monep)
- **Crypto** : oui — 11 ETF crypto éligibles
- **Fractions d'actions/ETF** : non
- **Investissement programmé (DCA)** : oui — à partir de 1 EUR, fréquence mensuelle ou trimestrielle, sur une sélection de 27 ETF et 40 actions ; 0 EUR de frais sur une sélection d'ETF ; disponible sur CTO et PEA
- **Rémunération des liquidités** : non
- **Plateforme de trading** : TradeBox (plateforme avancée), application mobile, ProRealTime et Wincharts 2.0 (payants)
- **Outils d'analyse** : consensus analystes, alertes, bulletin des marchés, séminaires et webinaires de formation
- **Note produits** : 3/5 *(source : sinvestir)*

## Propriété et transfert
<!-- Mis à jour : 2026-05-17 -->

- **Propriété des titres** : détention directe (titres détenus en nom propre)
- **Transfert de compte** : oui — 100 % des frais de transfert remboursés (jusqu'à 200 EUR pour CTO, 150 EUR pour PEA)
- **IFU fourni** : oui
- **Déclaration compte étranger** : non (compte domicilié en France)
- **Note propriété** : 5/5 *(source : sinvestir)*

## Conditions et contraintes spécifiques
<!-- Mis à jour : 2026-05-17 -->

- **Montant minimum d'un ordre** : 10 EUR à l'achat (CTO) ; 80 EUR pour PEA/PEA-PME/PEA-Jeunes
- **Types d'ordres** : marché, limite, stop, stop suiveur, à seuil de déclenchement
- **SRD (Service de Règlement Différé)** : disponible
- **Marchés accessibles** :
  - Euronext Paris, Amsterdam, Bruxelles
  - Euronext Growth Paris (ex-Alternext)
  - NYSE, NASDAQ (États-Unis)
  - LSE (Londres)
  - Xetra (Francfort)
  - Madrid
  - SIX / Virt-x (Suisse)
  - Euronext Lisbonne (Portugal)
- **Taxe sur les transactions financières (TTF)** : 0,40 % sur les achats au comptant de certaines actions françaises
- **Fiscalité des dividendes** : prélèvement forfaitaire de 31,40 % (12,8 % IR + 18,6 % PS) ; option pour le barème progressif possible
- **Fiscalité des plus-values** : flat tax 31,40 % dès le 1er euro ; option barème progressif avec abattements pour durée de détention (50 % de 2 à 8 ans, 65 % au-delà de 8 ans) pour titres acquis avant 2018

## Expérience client
<!-- Mis à jour : 2026-05-17 -->

- **Avis clients (Trustpilot)** : 2,6/5 (1 309 avis)
- **Contact** :
  - Téléphone : 01 56 88 40 40 (lun-ven 9h-20h, sam 11h-15h)
  - Email : via messagerie client (réservé aux clients)
  - Chat : non
  - Courrier : Bourse Direct — 374 rue Saint-Honoré, 75001 Paris
- **Agences physiques** : 4 agences (Paris, Lyon, Toulouse, Lille)
- **Test SAV** : attente de quelques secondes *(source : sinvestir)*
- **Note SAV** : Non communiqué

## Avis synthétique
<!-- Mis à jour : 2026-05-26 -->

- **Note globale** : 3,85/5 *(source : sinvestir)*
- **Points forts** :
  - Courtier français régulé (AMF/ACPR) avec 30 ans d'expérience
  - Frais de courtage très bas sur Euronext (0,99 EUR jusqu'à 500 EUR)
  - IFU fourni — pas de déclaration de compte étranger
  - Titres détenus en nom propre
  - 4 agences physiques en France
  - Investissement programmé (DCA) disponible dès 1 EUR
  - Large offre : + 10 000 actions, + 1 500 ETF, + 6 000 OPCVM
- **Points faibles** :
  - Plateforme moins agréable que certains concurrents
  - Service client moins bien réputé (Trustpilot 2,6/5)
  - Frais sur marchés étrangers (hors US) élevés
  - Pas de fractions d'actions/ETF
  - Pas de rémunération des liquidités
- **Services complémentaires** : assurance vie, PEA, PEA-PME, PEA Jeunes ; outils payants (ProRealTime, Wincharts 2.0, coaching bourse) ; séminaires et webinaires de formation ; bulletin des marchés

### Avis Avenue des Investisseurs
<!-- Mis à jour : 2026-05-26 -->
<!-- Source : Avenue des Investisseurs (mis à jour janvier 2026) — données subjectives/éditoriales -->

- Inclus dans le comparatif CTO ADI mais sans label distinctif — positionné comme un courtier français de base
- Recommandé uniquement pour les investisseurs centrés sur les valeurs de la zone Euronext (Paris, Bruxelles, Amsterdam) — les frais sur marchés étrangers (notamment US) le rendent peu compétitif pour le stock-picking international
- Apprécié pour sa régulation française et la simplicité administrative (IFU fourni, pas de déclaration de compte étranger, titres en nom propre)
- **Pour qui** : investisseurs français focalisés sur les marchés européens, cherchant un courtier hexagonal de longue date avec détention directe des titres — peu adapté aux investisseurs souhaitant des actions américaines ou une diversification internationale à moindre frais

## Sources

- [S'investir — Comparatif Compte-Titres](https://simulateurs.sinvestir.fr/les-comparateurs/compte-titres)
- [Bourse Direct — Tarifs Bourse](https://www.boursedirect.fr/fr/bourse/tarifs)
- [Bourse Direct — Grille tarifaire (PDF)](https://www.boursedirect.fr/pdf/tarifs_bd.pdf)
- [Bourse Direct — FAQ Compte-Titres](https://www.boursedirect.fr/fr/support/faq/compte-titres)
- [Bourse Direct — Investissement programmé](https://www.boursedirect.fr/fr/investissements-programmes)
- [Bourse Direct — Transfert](https://www.boursedirect.fr/fr/transfert)
- [Bourse Direct — Nos agences](https://www.boursedirect.fr/fr/nos-agences)
- [Avenue des Investisseurs — Comparatif CTO](https://avenuedesinvestisseurs.fr/comprendre-investir-bourse/cto-compte-titres-ordinaire/)
