---
nom: Fortuneo PER
type: contrat-per
slug: fortuneo
distributeur: Fortuneo
assureur: Suravenir (groupe Crédit Mutuel Arkéa)
source: simulateurs.sinvestir.fr
derniere-mise-a-jour: 2026-05-16
---

# Fortuneo PER

> PER individuel assurantiel distribué par [Fortuneo](https://www.fortuneo.fr/), assuré par Suravenir. Réservé aux détenteurs d'un compte courant Fortuneo. Voir aussi la fiche [PER (enveloppe)](../../enveloppes/per.md) pour les règles générales.

## Informations générales
<!-- Mis à jour : 2026-05-16 -->

- **Distributeur** : Fortuneo (filiale Crédit Mutuel Arkéa)
- **Assureur** : Suravenir — groupe Crédit Mutuel Arkéa (solvabilité : 263,7 %)
- **Type** : Assurantiel
- **Modes de gestion** : libre, gestion à horizon (3 profils)
- **Disponibilité** : 100 % en ligne (site + application mobile)
- **Versement minimum** : 100 EUR
- **Année de création** : 2025 (commercialisation restreinte en décembre 2024, lancement officiel le 5 février 2025)
- **Condition d'accès** : être client Fortuneo (détenir un compte courant)
- **Label** : Label d'Excellence — Les Dossiers de l'Epargne / Minalea

## Frais
<!-- Mis à jour : 2026-05-16 -->

- **Versement** : 0 %
- **Gestion UC (gestion libre)** : 0,50 %/an
- **Gestion UC (gestion à horizon)** : 0,70 %/an
- **Gestion fonds euros** : 0,70 %/an
- **Arbitrage** : 0 %
- **Frais d'entrée** : 0 %
- **Frais de sortie** : 0 % (hors sortie en rente)
- **Rente** : 0,80 % sur encours de rente
- **Surcoût gestion à horizon** : 0,20 %/an par rapport à la gestion libre (0,70 % vs 0,50 % sur UC)
- **Frais achat/vente ETF** : 0,10 %
- **Note frais** : 4,75/5

## Fonds euros
<!-- Mis à jour : 2026-05-16 -->

- **Fonds disponible** : Fonds Euros Retraite (garantie 99,3 % — 100 % max du contrat)
- **Performance 2024** : 3,20 %
- **Performance 2025** : 3,20 %
- **Performance cumulée 5 ans** : 10,95 %
- **Contraintes d'accès** : aucune — 100 % fonds euros possible (pas d'obligation d'investir en UC)
- **Stratégie** : allocation majoritairement obligataire, sécurité et récurrence du rendement
- **Note fonds euros** : 4,5/5

## Unités de compte
<!-- Mis à jour : 2026-05-16 -->

- **Nombre total d'UC** : + 220
- **ETF** : 11
- **SCPI** : 3 (Primovie, Patrimmo Commerce — Praemia REIM ; Transition Europe — Theoreim)
- **SCI** : 4 (Primonial Capimmo, Silver Avenir, Territoire Avenir, Tendance Pierre)
- **OPCI** : 1 (Opcimmo)
- **Titres vifs** : non
- **Rééquilibrage automatique** : oui (via options d'arbitrage automatique)
- **Redistribution loyers SCPI** : 85 % [à vérifier pour les nouveaux supports SCI/OPCI]
- **Autres supports** : fonds ISR, fonds thématiques, fonds actions, obligataires, diversifiés, fonds datés
- **Note UC** : 3,5/5

## Conditions et contraintes spécifiques
<!-- Mis à jour : 2026-05-16 -->

### Versements

- **Versement initial minimum** : 100 EUR
- **Versements libres minimum** : 100 EUR [à vérifier]
- **Versements programmés minimum** : 50 EUR/mois
- **Versements programmés sur SCPI** : non autorisés

### Arbitrages

- **Arbitrage minimum** : 100 EUR
- **Frais d'arbitrage** : 0 %
- **Changement de mode de gestion** : gratuit, à tout moment

### Options d'arbitrage automatique

- **Investissement progressif** : arbitrage programmé (mensuel, trimestriel, semestriel, annuel) — montant min. 100 EUR, déclenchement si encours ≥ 1 200 EUR sur les supports concernés — gratuit
- **Stop-loss relatif (seuil évolutif)** : transfert automatique vers support sécurisé en cas de baisse — seuil min. 5 % — montant min. à arbitrer 100 EUR — gratuit
- **Sécurisation des plus-values** : transfert automatique des gains vers support éligible — seuil min. 5 % de plus-value du capital net investi — montant min. 100 EUR — gratuit
- **Dynamisation des plus-values du fonds euros** : arbitrage annuel automatique des gains du Fonds Euros Retraite vers les UC choisies — montant min. 100 EUR — gratuit

### Supports immobiliers — Règles spécifiques

- **Limite par support** : 50 000 EUR max et 50 % de l'encours total du contrat [à vérifier — données issues du contrat Fortuneo Vie, applicabilité au PER à confirmer]
- **Valorisation SCPI** : hebdomadaire (1er jour ouvré de chaque semaine)
- **Revenus SCPI** : réinvestis mensuellement sur le fonds euros du contrat
- **Versements programmés** : non autorisés sur les SCPI, SCI et OPCI

### Gestion a horizon

- **Mode par défaut** : oui — gestion à horizon activée à la souscription
- **Profils** : Prudent, Equilibré (défaut), Dynamique
- **Profil Prudent** : exposition limitée aux actifs risqués, priorité à la stabilité — 2,67 % net en 2025 — performance annualisée depuis création : 2,05 % net (horizon 2+ ans)
- **Profil Equilibré** : diversification entre sécurité et performance, risque modéré — 3,90 % net en 2025 — performance annualisée depuis création : 2,62 % net (horizon 2+ ans)
- **Profil Dynamique** : forte exposition actions, espérance de rendement supérieure — 4,66 % net en 2025 — performance annualisée depuis création : 2,83 % net (horizon 2+ ans)
- **Dimension responsable** : ≥ 80 % de fonds ayant des objectifs d'investissement durable (SFDR)
- **Changement de profil** : gratuit, à tout moment
- **Frais de gestion** : 0,70 %/an sur les UC (vs 0,50 % en gestion libre)
- **Basculement vers gestion libre** : possible à tout moment, sans frais

### Transfert

- **Transfert entrant** : possible depuis PERP, Madelin, PERCO, Article 83, autres PER — remboursement des frais de transfert jusqu'à 1 000 EUR pour un encours min. de 5 000 EUR
- **Bonus ouverture** : 100 EUR pour un versement initial ≥ 2 000 EUR ; 200 EUR pour un versement initial ≥ 4 000 EUR (offre valable du 04/05/2026 au 30/06/2026)
- **Transfert sortant** : 1 % max si contrat < 5 ans ; gratuit si contrat ≥ 5 ans (encadrement légal)

## Liste des ETF disponibles
<!-- Mis à jour : 2026-05-16 -->
<!-- Source : https://www.fortuneo.fr/per-plan-epargne-retraite/gestion-libre -->
<!-- Source : https://mabanque.fortuneo.fr/document-assurance-vie/PERFSUPPORTPER -->
<!-- Source : https://intra.suravenir.fr/web/suravenir.fr/perfs-des-uc-fortuneo-vie -->

| Nom | ISIN | Catégorie | Frais internes |
|---|---|---|---|
| Amundi MSCI World II UCITS ETF Dist | FR0010315770 | Actions Monde | 0,30 %/an |
| Amundi Core S&P 500 Swap UCITS ETF EUR Dist | LU0496786574 | Actions USA | 0,05 %/an |
| Amundi CAC 40 UCITS ETF Dist | FR0007052782 | Actions France | 0,25 %/an |
| Amundi EURO STOXX 50 II UCITS ETF Acc | FR0007054358 | Actions Zone Euro | 0,20 %/an |
| Amundi STOXX Europe 600 ESG UCITS ETF DR EUR (C) | LU1684369710 | Actions Europe | 0,08 %/an |
| Amundi MSCI Emerging Markets Swap II UCITS ETF EUR Acc | FR0010429068 | Actions Emergents | 0,55 %/an |
| Amundi Nasdaq-100 II UCITS ETF Acc | LU1829221024 | Actions USA Tech | 0,20 %/an |
| Amundi MSCI World SRI Climate Net Zero Ambition PAB UCITS ETF EUR Acc | IE000Y77LGG9 | Actions Monde SRI/Climat | 0,08 %/an |
| Amundi MSCI Water ESG Screened UCITS ETF Dist | FR0010527275 | Actions Monde Thématique Eau | 0,60 %/an |
| Amundi Global Equity Quality Income UCITS ETF Dist | LU0832436512 | Actions Monde Dividendes | 0,35 %/an |
| Amundi Bloomberg Equal-weight Commodity ex-Agriculture UCITS ETF Acc | LU1829218749 | Matières premières | 0,25 %/an |

- **Frais achat/vente ETF** : 0,10 % par opération (prélevé par Fortuneo en sus des frais internes du fonds)
- **Frais de gestion UC** : 0,50 %/an en gestion libre (prélevé par l'assureur, s'ajoute aux frais internes)

## Liste des supports immobiliers disponibles
<!-- Mis à jour : 2026-05-16 -->
<!-- Source : https://www.fortuneo.fr/assurance-vie/scpi-immobilier -->

### SCPI

| Nom | ISIN | Société de gestion | Perf. 2024 | Risque |
|---|---|---|---|---|
| Primovie | QS0002005324 | Praemia REIM | +2,94 % | 3/7 |
| Patrimmo Commerce | QS0002005299 | Praemia REIM | -8,01 % | 3/7 |
| Transition Europe | FR001400EGA8 | Theoreim | +7,50 % | 3/7 |

### SCI

| Nom | ISIN | Perf. 2024 | Risque |
|---|---|---|---|
| Primonial Capimmo | QS0002005277 | -14,05 % | 2/7 |
| Silver Avenir | FR0013526100 | +3,28 % | 3/7 |
| Territoire Avenir | FR001400A217 | +3,83 % | 2/7 |
| Tendance Pierre | FR0014007X71 | N/C | 2/7 |

### OPCI

| Nom | ISIN | Perf. 2024 | Risque |
|---|---|---|---|
| Opcimmo | FR0011066802 | -8,88 % | 4/7 |

- **Versements programmés sur supports immobiliers** : non autorisés
- **Revenus SCPI** : réinvestis mensuellement sur le fonds euros du contrat [à vérifier pour SCI/OPCI]
- **Contraintes de plafonnement** : 50 000 EUR max et 50 % de l'encours total par support [à vérifier — données issues du contrat Fortuneo Vie, applicabilité au PER à confirmer]

## Expérience client
<!-- Mis à jour : 2026-05-16 -->

- **Avis clients** : 3,9/5 (14 756 avis — tous produits Fortuneo confondus)
- **App mobile** : oui
- **Contact** :
  - Téléphone : 02 98 00 29 00
  - Email : réservé aux clients
  - Chat : oui
  - Formulaire de contact : oui
  - Courrier : oui
- **Ergonomie** : simple et efficace
- **Note SAV** : 3,9/5

## Avis synthétique
<!-- Mis à jour : 2026-05-16 -->

- **Note globale** : 4,13/5
- **Points forts** :
  - Accessible dès 100 EUR (un des seuils les plus bas du marché)
  - 0 % frais de versement, d'entrée, d'arbitrage et de sortie
  - Fonds euros performant (3,20 % net en 2025) accessible à 100 % sans contrainte UC
  - Gestion à horizon responsable (≥ 80 % fonds durables) avec 3 profils
  - Performances gestion à horizon solides (jusqu'à 4,66 % en 2025 pour le profil Dynamique)
  - Options d'arbitrage automatique gratuites (stop-loss, sécurisation, dynamisation)
  - Versements programmés dès 50 EUR/mois
  - Solvabilité élevée de l'assureur (263,7 %)
- **Points faibles** :
  - Seulement 3 SCPI disponibles (diversification géographique limitée)
  - Seulement 11 ETF (offre limitée vs concurrence)
  - Pas de titres vifs
  - Redistribution SCPI à 85 % seulement (vs 100 % chez certains concurrents)
  - Réservé aux clients Fortuneo (nécessite un compte courant)
  - Pas de gestion pilotée sous mandat par un gérant externe
  - + 220 UC seulement (vs + 700 chez certains concurrents)
- **Services complémentaires** : CTO, PEA, assurance-vie (Fortuneo Vie), banque en ligne

## Sources

- [S'investir -- Comparatif PER](https://simulateurs.sinvestir.fr/les-comparateurs/per)
- [Fortuneo -- PER](https://www.fortuneo.fr/per-plan-epargne-retraite)
- [Fortuneo -- Gestion libre PER](https://www.fortuneo.fr/per-plan-epargne-retraite/gestion-libre)
- [Fortuneo -- Gestion a horizon PER](https://www.fortuneo.fr/per-plan-epargne-retraite/gestion-horizon)
- [Fortuneo -- Options de gestion PER](https://www.fortuneo.fr/per-plan-epargne-retraite/options)
- [Fortuneo -- Transfert PER](https://www.fortuneo.fr/blog/transfert-per)
- [Suravenir -- Lancement Fortuneo PER](https://www.suravenir.fr/lancement-fortuneo-per/)
- [Fortuneo -- Supports immobiliers (SCPI, SCI, OPCI)](https://www.fortuneo.fr/assurance-vie/scpi-immobilier)
- [Fortuneo -- SCPI éligibles (AV)](https://www.fortuneo.fr/faq/quelles-sont-les-scpi-eligibles-au-contrat-fortuneo-vie)
- [Suravenir -- Performances UC Fortuneo](https://intra.suravenir.fr/web/suravenir.fr/perfs-des-uc-fortuneo-vie)
- [Fortuneo -- Performance supports PER](https://mabanque.fortuneo.fr/document-assurance-vie/PERFSUPPORTPER)
