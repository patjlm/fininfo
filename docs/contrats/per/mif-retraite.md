---
nom: MIF Retraite
type: contrat-per
slug: mif-retraite
distributeur: MIF
assureur: MIF
frais-versement: "0 %"
frais-gestion-uc: "0,60 %"
frais-arbitrage: "0 % (1 % vers fonds euros)"
source: simulateurs.sinvestir.fr
derniere-verification: 2026-05-26
---

# MIF Retraite

> PER individuel assurantiel distribué et assuré par la [MIF](https://www.mifassur.com/) (Mutuelle d'Ivry la Fraternelle). Contrat groupe souscrit par l'ADERM (Association pour le Développement de l'Épargne Retraite Mutualiste). Voir aussi la fiche [PER (enveloppe)](../../enveloppes/per.md) pour les règles générales.

## Informations générales
<!-- Mis à jour : 2026-05-26 -->

- **Distributeur** : MIF (Mutuelle d'Ivry la Fraternelle)
- **Assureur** : MIF (solvabilité : 229 %)
- **Type** : Assurantiel (contrat groupe)
- **Modes de gestion** : libre, pilotée à horizon (par défaut), sous mandat
- **Disponibilité** : en ligne et en agence
- **Versement minimum** : 500 EUR en gestion libre, sous mandat ou à horizon
- **Versements programmés** : dès 30 EUR/mois
- **Année de création** : 2021
- **Âge maximum de souscription** : 67 ans
- **Adhésion** : 10 EUR (frais ADERM)

## Frais
<!-- Mis à jour : 2026-05-26 -->

- **Versement** : 0 %
- **Gestion UC (gestion libre)** : 0,60 %/an
- **Gestion UC (gestion sous mandat)** : 0,80 %/an
- **Gestion fonds euros** : 0,60 %/an
- **Arbitrage** : 0 % (gratuit, y compris en gestion sous mandat)
- **Rente (arrérages)** : 1 %
- **Surcoût gestion sous mandat** : 0,20 %/an (inclus dans les 0,80 % UC sous mandat)
- **Frais d'adhésion ADERM** : 10 EUR (unique)
- **Note frais** : 4,5/5

## Fonds euros
<!-- Mis à jour : 2026-05-26 -->

- **Fonds disponible** : Fonds euros MIF Retraite (actif cantonné, garanti à 100 % — 70 % max du contrat)
- **Performance 2022** : 2,50 %
- **Performance 2023** : 3,15 %
- **Performance 2024** : 3,45 %
- **Performance 2025** : 3,55 %
- **Performance cumulée 5 ans** : 14,35 %
- **Objectif 2026** : 4,60 % net (non garanti, avec bonification de 1,05 % sous conditions — offre du 01/04/2026 au 31/07/2026)
- **Contraintes d'accès** : obligation d'investir 30 % minimum en unités de compte — placement à 100 % en fonds euros impossible
- **Effet cliquet** : oui — intérêts définitivement acquis chaque année
- **Allocation** : 79,5 % obligations (au 31/12/2025)
- **Provision pour participation aux bénéfices (PPE)** : 1,10 % des actifs
- **Note fonds euros** : 5/5

## Unités de compte
<!-- Mis à jour : 2026-05-26 -->

- **Nombre total d'UC** : 37 (approche sélective "best in class")
- **ETF** : 7
- **SCPI** : 4
- **Titres vifs** : non
- **Rééquilibrage automatique** : oui
- **Redistribution loyers SCPI** : 90 % (réinvestis sur le fonds euros Retraite du contrat)
- **Autres supports** : 2 fonds Private Equity, fonds ISR (article 8 et 9 SFDR), fonds structurés, fonds patrimoniaux, fonds obligataires et monétaires
- **Note UC** : 2,5/5

## Conditions et contraintes spécifiques
<!-- Mis à jour : 2026-05-26 -->

### Versements

- **Versement initial minimum** : 500 EUR (gestion libre, sous mandat ou à horizon)
- **Versements libres minimum** : 500 EUR [à vérifier]
- **Versements programmés minimum** : 30 EUR/mois
- **Obligation UC** : tout versement doit comporter au minimum 30 % en unités de compte

### Arbitrages

- **Frais d'arbitrage** : 0 % (gratuit)
- **Arbitrage vers fonds euros** : 1 % de frais (confirmé site officiel)
- **Arbitrage minimum** : non communiqué

### SCPI — Règles spécifiques

- **SCPI disponibles** : 4 SCPI (Cristal Life, Cristal Rente, [Épargne Pierre](../../actifs/scpi/epargne-pierre.md), [Immorente](../../actifs/scpi/immorente.md))
- **Redistribution loyers** : 90 % seulement (vs 100 % chez certains concurrents)
- **Réinvestissement loyers** : sur le fonds euros Retraite (actif cantonné)
- **Fréquence** : trimestrielle

### Gestion pilotée à horizon (mode par défaut)

- **Profils disponibles** : Prudent, Équilibré (par défaut), Dynamique, Offensif
- **Allocation** : répartition entre poche « Sérénité » (fonds euros + fonds obligataire DCNA Sérénité Plus) et poches « Performance » et « Alternative » (UC)
- **Désensibilisation** : arbitrages automatiques trimestriels vers le fonds euros à l'approche de la retraite
- **Grille indicative profil Équilibré** :
  - Plus de 10 ans de la retraite : 0 % minimum en supports à faible risque
  - 5 à 10 ans : 20 % en supports à faible risque
  - 2 à 5 ans : 50 % en supports à faible risque
  - Moins de 2 ans : 70 % en supports à faible risque
- **Minimum** : 500 EUR
- **Frais de gestion** : 0,60 %/an (pas de surcoût vs gestion libre)

### Gestion sous mandat

- **Gestionnaire** : OFI Invest AM (sur les conseils de la MIF)
- **Profils disponibles** : Prudent (70/30), Équilibré (50/50), Dynamique (30/70), Offensif (10/90) — ratio fonds euros / UC
- **Performances 2025** : Prudent 3,98 % — Équilibré 4,52 % — Dynamique 5,13 % — Offensif 6,29 %
- **Supports** : 100 % labellisés ISR (article 8 ou 9 SFDR), 46 supports sélectionnés
- **Surcoût** : 0,20 %/an (total 0,80 % gestion UC sous mandat)
- **Minimum** : 500 EUR
- **Fréquence des arbitrages** : mensuels (autant que nécessaire)
- **Suivi** : rapport mensuel + rapport trimestriel détaillé

### Transfert

- **Transfert entrant** : accepté (anciens PERP, Madelin, Perco, article 83 ; frais jusqu'à 5 % chez l'ancien assureur)
- **Transfert sortant** : 1 % max si contrat < 5 ans ; gratuit si contrat >= 5 ans (encadrement légal, loi PACTE)
- **Transfert sortant 2025+** : possiblement 0 % quelle que soit l'ancienneté [à vérifier]

## Liste des ETF disponibles
<!-- Mis à jour : 2026-05-26 -->
<!-- Source : https://www.mifassur.com/retraite/mif-per-retraite/gestion-libre/ -->
<!-- Note : site mifassur.com indisponible (504) le 2026-05-26 — liste non re-vérifiable ce jour -->

[liste partielle — 6 supports sur 7 identifiés avec ISIN — le site mifassur.com charge la liste dynamiquement (JavaScript), non extractible par WebFetch]

| Nom | ISIN | Catégorie | Frais spécifiques |
|---|---|---|---|
| Amundi CAC 40 UCITS ETF Acc | [FR0013380607](../../actifs/etf/FR0013380607.md) | Actions France | 0,25 %/an, Article 6 |
| Amundi EUROSTOXX 50 II UCITS ETF Acc | [FR0007054358](../../actifs/etf/FR0007054358.md) | Actions Zone Euro | 0,20 %/an, Article 6, SRI 5 |
| iShares Core S&P 500 UCITS ETF | [IE00B5BMR087](../../actifs/etf/IE00B5BMR087.md) | Actions États-Unis | 0,07 %/an, Article 6, SRI 4 |
| iShares Core MSCI World UCITS ETF | [IE00B4L5Y983](../../actifs/etf/IE00B4L5Y983.md) | Actions International | 0,20 %/an, Article 6, SRI 3 |
| BNP Paribas Easy CAC 40 ESG UCITS ETF | [FR0010150458](../../actifs/etf/FR0010150458.md) | Actions France ESG | 0,25 %/an, Article 8 |
| Amundi MSCI Water ESG Screened UCITS ETF Dist | [FR0010527275](../../actifs/etf/FR0010527275.md) | Actions Thématique Eau | 0,60 %/an, Article 8 |
| [ETF à identifier] | [ISIN à compléter] | [à compléter] | [à compléter] |

## Liste des SCPI disponibles
<!-- Mis à jour : 2026-05-26 -->
<!-- Source : https://www.mifassur.com/retraite/mif-per-retraite/investir-en-scpi-per/ -->
<!-- Note : site mifassur.com indisponible (504) le 2026-05-26 — liste non re-vérifiable ce jour -->

| Nom | Société de gestion | Perf. 2025 | Perf. 2024 | Perf. 2023 | Contraintes spécifiques |
|---|---|---|---|---|---|
| Cristal Life | Inter Gestion REIM | 6,54 % | 6,51 % | 6,02 % | 90 % loyers reversés trimestriellement sur fonds euros Retraite, labellisée ISR |
| Cristal Rente | Inter Gestion REIM | 5,00 % | 5,06 % | 5,00 % | 90 % loyers reversés trimestriellement sur fonds euros Retraite |
| [Épargne Pierre](../../actifs/scpi/epargne-pierre.md) | Atland Voisin | 5,28 % | 5,28 % | 5,28 % | 90 % loyers reversés trimestriellement sur fonds euros Retraite |
| [Immorente](../../actifs/scpi/immorente.md) | Sofidy | 5,00 % | 5,04 % | 5,00 % | 90 % loyers reversés trimestriellement sur fonds euros Retraite |

## Expérience client
<!-- Mis à jour : 2026-05-26 -->

- **Avis clients** : 2,7/5 (32 avis)
- **App mobile** : oui
- **Contact** :
  - Téléphone : 09 70 15 77 77
  - Chat : oui
  - Email : non communiqué
  - Courrier : non communiqué
- **Ergonomie** : simple et efficace
- **Note SAV** : 4/5

## Avis synthétique
<!-- Mis à jour : 2026-05-26 -->

- **Note globale** : 4,2/5
- **Points forts** :
  - Fonds euros très performant (3,55 % en 2025, 14,35 % cumulés sur 5 ans) avec actif cantonné
  - 0 % de frais sur les versements et les arbitrages
  - Frais de gestion compétitifs (0,60 %/an)
  - Solvabilité très élevée (229 %)
  - Gestion sous mandat 100 % ISR avec 4 profils
  - Accessible en ligne et en agence, dès 30 EUR/mois
  - Contrat primé par la presse spécialisée (Victoire du Particulier 2025, Grand Prix Mieux Vivre votre Argent)
- **Points faibles** :
  - Gamme d'UC réduite (37 supports seulement vs 700+ chez certains concurrents)
  - Seulement 7 ETF
  - Seulement 4 SCPI avec 90 % des loyers reversés (vs 100 % chez la concurrence)
  - Impossible de placer 100 % en fonds euros (30 % UC minimum obligatoire)
  - Pas de titres vifs
  - Gestion sous mandat avec OPCVM classiques (pas d'ETF)
  - Avis clients mitigés (2,7/5)
- **Services complémentaires** : assurance-vie (Compte Épargne Libre Avenir Multisupport), prévoyance

## Sources

- [S'investir — Comparatif PER](https://simulateurs.sinvestir.fr/les-comparateurs/per)
- [MIF — MIF PER Retraite](https://www.mifassur.com/retraite/mif-per-retraite/)
- [MIF — Gestion libre PER](https://www.mifassur.com/retraite/mif-per-retraite/gestion-libre/)
- [MIF — Gestion sous mandat PER](https://www.mifassur.com/retraite/mif-per-retraite/gestion-sous-mandat/)
- [MIF — Gestion à horizon PER](https://www.mifassur.com/retraite/mif-per-retraite/gestion-a-horizon/)
- [MIF — Frais contrats assurance-vie et PER](https://www.mifassur.com/frais-contrats-assurance-vie-et-per-mif/)
- [MIF — SCPI PER](https://www.mifassur.com/retraite/mif-per-retraite/investir-en-scpi-per/)
- [MIF — Fonds en euros retraite](https://www.mifassur.com/retraite/mif-per-retraite/fonds-en-euros-retraite/)
- [MIF — Taux 2025](https://www.mifassur.com/performance/taux-mif/)
