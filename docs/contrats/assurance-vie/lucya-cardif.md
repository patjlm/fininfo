---
nom: Lucya Cardif
type: contrat-assurance-vie
slug: lucya-cardif
distributeur: Lucya
assureur: BNP Paribas Cardif (Cardif Assurance Vie)
frais-versement: "0 %"
frais-gestion-uc: "0,50 %"
frais-arbitrage: "0 % (0,10 % sur ETF)"
derniere-verification: 2026-05-26
---

# Lucya Cardif

> Assurance vie distribuée par Lucya, assurée par BNP Paribas Cardif (filiale de BNP Paribas).

Voir aussi : [Enveloppe Assurance Vie](../../enveloppes/assurance-vie.md) pour les règles générales.

## Informations générales
<!-- Mis à jour : 2026-05-26 -->

- **Distributeur** : Lucya (plateforme en ligne, CIF réglementé AMF)
- **Assureur** : BNP Paribas Cardif (Cardif Assurance Vie — filiale de BNP Paribas — CA 19,1 Md€ en 2025 — UC 72,4 Md€ + fonds général 101,4 Md€ — ratio SCR : 170 % au 31/12/2025)
- **Type** : Multisupports groupé
- **Modes de gestion** : Libre, déléguée (3 profils)
- **Disponibilité** : En ligne exclusivement (souscription sur assurancevie.com)
- **Versement minimum** : 500 EUR (gestion libre) / 1 000 EUR (gestion déléguée)
- **Année de création** : 2001

## Frais
<!-- Mis à jour : 2026-05-26 -->

- **Versement** : 0 %
- **Gestion UC** : 0,50 % / an
- **Gestion fonds euros (Fonds général)** : 0,70 % / an
- **Gestion fonds euros (Fonds Euro Private Strategies)** : 3 % / an maximum
- **Arbitrage** : 0 % (illimité, sans minimum)
- **Arbitrage ETF** : 0,10 % par transaction
- **Surcoût gestion déléguée** : +0,25 % / an (maximum 0,50 %)
- **Frais de rachat** : 0 %
- **Frais d'adhésion** : 10 EUR (paiement unique à l'ouverture, association souscripteurs)
- **Pénalités SCPI** : 3 % sur les sommes rachetées ou arbitrées sur les supports SCPI dans les 3 ans suivant l'investissement sur ce support
- **Frais de rente** : 2,75 % sur les versements de rente
- **Note frais** : 4,5/5 (source : sinvestir.fr)

## Fonds euros
<!-- Mis à jour : 2026-05-26 -->

- **Fonds Euro Général** (garanti à 99,3 % — 100 % possible)
  - Performance 2024 : 2,75 %
  - Performance 2025 : 2,75 %
  - Performance cumulée 5 ans : 11,60 %
  - Contraintes d'accès : aucune contrainte particulière ; 100 % fonds euros possible sans condition d'UC minimale
  - Bonus 2026-2027 : jusqu'à +1,50 % (hypothèse de taux 4,25 %) — conditions : minimum 10 000 EUR versés et 45 % de l'encours en UC — non garanti (source : lucya.com/rendement-des-fonds-euros-2025/)
- **Fonds Euro Private Strategies** (garanti à 97 % — actifs non cotés)
  - Performance 2024 : 3,00 %
  - Performance 2025 : 2,75 %
  - Performance cumulée 5 ans : 14,27 %
  - Contraintes d'accès : accessible uniquement par versements libres (non programmés) — montant minimum 500 EUR — le montant investi en UC doit être au moins 2 fois supérieur au montant investi dans ce fonds (ce fonds ne peut donc pas dépasser 1/3 d'un versement) — ne peut pas représenter plus de 1/3 de l'encours total du contrat
  - Frais de gestion : 3 % / an (inclus dans la performance nette affichée)
- **Note fonds euros** : 4,25/5 (source : sinvestir.fr)

## Unités de compte
<!-- Mis à jour : 2026-05-26 -->

- **Nombre total d'UC** : +2 300
- **ETF** : ~80 (couvrant les principaux indices boursiers)
- **OPCVM** : ~1 000
- **SCPI** : 3 (avec redistribution des loyers à 100 %) — à fenêtre de commercialisation
- **SC / SCI / OPCI** : oui — à fenêtre de commercialisation
- **Titres vifs (actions)** : oui — plus de 1 000 titres (seuil minimum : 3 000 EUR par titre [à vérifier] — accessible 30 jours après validation du contrat par l'assureur)
- **Private equity** : oui (fonds avec fenêtres de souscription)
- **Obligations / fonds obligataires** : oui
- **Produits structurés** : oui
- **Rééquilibrage automatique** : oui
- **Arbitrage progressif** : oui (transfert automatique progressif entre supports)
- **Stop-loss absolu et relatif** : oui
- **Versements programmés sur ETF** : oui (frais de 0,10 % par opération)
- **Note UC** : 5/5 (source : sinvestir.fr)

## Conditions et contraintes spécifiques
<!-- Mis à jour : 2026-05-26 -->

- **Versement initial minimum** : 500 EUR (gestion libre) / 1 000 EUR (gestion déléguée)
- **Versements libres minimum** : 500 EUR
- **Versements programmés minimum** : 50 EUR / mois — 150 EUR / trimestre — 300 EUR / semestre — 600 EUR / an
- **Arbitrage minimum** : pas de montant minimum
- **Rachat partiel minimum** : 100 EUR (ponctuel) — 100 EUR / mois (programmé)
- **Titres vifs** : accessibles 30 jours après validation du contrat par l'assureur — seuil minimum par titre : 3 000 EUR [à vérifier]
- **Règles d'arbitrage** :
  - Arbitrage gratuit et illimité entre tous supports (sans minimum)
  - ETF : 0,10 % de frais par opération d'arbitrage (achat, vente ou arbitrage)
  - SCPI : pénalité de 3 % sur les sommes arbitrées dans les 3 ans suivant l'investissement sur ce support
- **Limites d'investissement** :
  - Fonds Euro Général : 100 % possible, sans contrainte
  - Fonds Euro Private Strategies : ne peut pas dépasser 1/3 du contrat ; l'investissement en UC au moment du versement doit être au moins 2x le montant investi dans ce fonds ; accessible uniquement par versements libres (non programmés)
- **Conditions gestion déléguée** :
  - Minimum : 1 000 EUR
  - 3 profils disponibles : Modéré (horizon 3 ans, risque faible), Équilibré (horizon 5 ans), Dynamique (horizon 8 ans, 40-100 % actions)
  - Frais supplémentaires : +0,25 % / an (maximum 0,50 %)
  - Changement de profil ou de mode de gestion : gratuit, à tout moment
  - Performances 2024 : Modéré +9,66 % — Équilibré +9,94 % — Dynamique +10,95 %
  - Performances 2025 : Modéré +3,13 % — Équilibré +5,10 % — Dynamique +7,74 %
  - Performance depuis origine (Modéré, depuis 06/2023) : +7,92 % cumulé (à mars 2026)
- **Avance sur contrat** : disponible (sous conditions de l'assureur)
- **Clause bénéficiaire** : standard / libre / démembrée

### SCPI — Règles spécifiques

- **SCPI disponibles** : 3 (à fenêtre de commercialisation — disponibilité variable)
- **Pénalité de sortie** : 3 % sur les sommes rachetées ou arbitrées dans les 3 ans suivant l'investissement sur ce support
- **Versements programmés sur SCPI** : [à vérifier — non mentionné dans la documentation disponible]
- **Redistribution des loyers** : 100 %
- **Limite par versement** : [à vérifier — pas de limite explicite mentionnée pour les SCPI dans les conditions publiées]

### Fonds euros — Règles spécifiques

- **Fonds Euro Général** : 100 % possible sans contrainte minimale d'UC — taux unique non conditionné à la part d'UC (2,75 % net en 2025)
- **Bonus 2026-2027** : jusqu'à +1,50 % (hypothèse 4,25 %) — conditions : minimum 10 000 EUR et 45 % de l'encours en UC — non garanti (source : lucya.com/rendement-des-fonds-euros-2025/)
- **Fonds Euro Private Strategies** : accessible uniquement par versements libres (non programmés), montant minimum 500 EUR — le montant en UC au moment du versement doit être au moins 2 fois le montant investi dans ce fonds — ne peut pas dépasser 1/3 de l'encours total du contrat — frais de gestion 3 % / an (inclus dans la performance nette affichée)
- **Garantie Fonds Euro Général** : 99,3 % (garantie permanente du capital hors frais de gestion de 0,70 % / an)
- **Garantie Fonds Euro Private Strategies** : 97 % du capital à terme

## Fiscalité
<!-- Mis à jour : 2026-05-26 -->

- **Avant 8 ans** : PFU 30 % (12,8 % IR + 17,2 % PS) ou barème IR + PS
- **Après 8 ans** : abattement annuel 4 600 EUR (célibataire) / 9 200 EUR (couple) puis 24,7 % (7,5 % + 17,2 % PS) ou barème IR + PS
- **Transmission** : abattement 152 500 EUR / bénéficiaire (versements avant 70 ans) ; 30 500 EUR tous bénéficiaires confondus (versements après 70 ans)

## Expérience client
<!-- Mis à jour : 2026-05-26 -->

- **Avis clients** : 4,3/5 (15 avis — source : sinvestir.fr)
- **App mobile** : non
- **Contact** : téléphone (01 44 76 87 60 — lun-ven 9h-18h30, sam 9h30-13h), email (contact@lucya.com), courrier (Lucya — 13 rue d'Uzès 75002 Paris), formulaire de contact
- **Ergonomie** : simple et efficace ; souscription et gestion en ligne via assurancevie.com
- **Note SAV** : 4,75/5 (source : sinvestir.fr)

## Avis synthétique
<!-- Mis à jour : 2026-05-26 -->

### Avis Avenue des Investisseurs
<!-- Mis à jour : 2026-05-26 -->
<!-- Source : Avenue des Investisseurs (mis à jour 17 septembre 2025) — données subjectives/éditoriales -->

- L'un des meilleurs contrats pour la gestion libre en bourse selon ADI : large offre d'ETF, de fonds et de titres vifs
- Fonds euro bien noté par ADI pour sa rémunération compétitive
- Particularité exclusive : accepte les US Persons (doubles nationaux franco-américains), ce qui en fait souvent le seul choix disponible pour ce profil
- ADI signale un service client parfois difficile à joindre comme principal point faible
- **Pour qui** : investisseurs souhaitant gérer librement un portefeuille diversifié en bourse, US Persons, profils cherchant un maximum de supports disponibles

- **Note globale** : 4,65/5 (source : sinvestir.fr)
- **Points forts** :
  - Un des meilleurs contrats pour la gestion en ETF (~80 ETF disponibles)
  - 100 % fonds euros possible sans contrainte sur le Fonds Général
  - Deux fonds euros accessibles avec performances compétitives (2,75 % net en 2025)
  - Plus de 2 300 UC au total, dont titres vifs
  - 0 % de frais de versement et d'arbitrage (hors ETF)
  - Gestion déléguée accessible dès 1 000 EUR avec 3 profils clairs
  - Assureur solide : BNP Paribas Cardif (filiale BNP Paribas)
  - Redistribution 100 % des loyers SCPI
- **Points faibles** :
  - Peu de SCPI disponibles (3 seulement)
  - Pas d'ETF Russell 2000 ni Emerging Markets non-ISR (hors catégorie ISR)
  - Pénalité de 3 % sur les rachats et arbitrages SCPI dans les 3 premières années
  - Frais de gestion du Fonds Euro Private Strategies élevés (3 % / an)
  - Contrainte d'accès au Fonds Euro Private Strategies complexe (ratio UC 2:1)
  - Pas d'application mobile
  - 10 EUR de frais d'adhésion à l'ouverture
- **Services complémentaires** : PER Lucya Cardif, PEA assurantiel, Contrat de capitalisation, SCPI (hors assurance-vie)

## Sources

- [Avenue des Investisseurs — Comparatif Assurance Vie](https://www.avenuedesinvestisseurs.fr/comparatif-assurance-vie/) (mis à jour 17 septembre 2025)
- [Lucya — Page produit Lucya Cardif](https://www.lucya.com/lucya-cardif/)
- [Lucya — Comparateur des contrats](https://www.lucya.com/comparer-nos-assurances-vie-lucya/)
- [Lucya — Gestion déléguée](https://www.lucya.com/gestion-deleguee/)
- [Lucya — Rendement fonds euros 2025](https://lucya.com/rendement-des-fonds-euros-2025/)
- [Assurancevie.com — Liste des supports Lucya Cardif](https://www.assurancevie.com/asv/nos-contrats/lucya-cardif-2025/liste-supports)
- [BNP Paribas Cardif — Site institutionnel](https://www.bnpparibascardif.com)
- [BNP Paribas Cardif — Informations financières](https://www.bnpparibascardif.com/investisseurs/informations-financieres-2/)
- [Cardif Assurance Vie — SFCR 2025 (ratio SCR 170 %)](https://www.bnpparibascardif.com/wp-content/uploads/sites/28/2026/04/Cardif-Assurance-Vie-–-SFCR-2025.pdf)
- [S'investir — Comparatif Assurance Vie](https://simulateurs.sinvestir.fr/les-comparateurs/assurance-vie)
