---
nom: Finary Life
type: contrat-assurance-vie
slug: finary-life
distributeur: Finary
assureur: Generali Vie
frais-versement: "0 %"
frais-gestion-uc: "0,50 %"
frais-arbitrage: "0 %"
derniere-verification: 2026-05-26
---

# Finary Life

> Assurance vie distribuée par Finary (courtier ORIAS 21001279), assurée par Generali Vie — contrat dénommé commercialement « e-vie ».

Voir aussi : [Enveloppe Assurance Vie](../../enveloppes/assurance-vie.md) pour les règles générales.

## Informations générales
<!-- Mis à jour : 2026-05-26 -->

- **Distributeur** : Finary SAS (agrégateur patrimonial et courtier en assurance, ORIAS 21001279)
- **Assureur** : Generali Vie (groupe Generali, 8 millions d'assurés en France, surveillé par l'ACPR — solvabilité : Non communiqué)
- **Nom du contrat** : e-vie (nom commercial : Finary Life)
- **Type** : Multisupports individuel
- **Modes de gestion** : Libre, profilée (3 profils)
- **Disponibilité** : En ligne exclusivement
- **Versement minimum** : 300 EUR (sans versements programmés) / 150 EUR (avec versements programmés mensuels)
- **Année de création** : 2020

## Frais
<!-- Mis à jour : 2026-05-26 -->

- **Versement** : 0 %
- **Gestion UC** : 0,50 % / an
- **Gestion fonds euros (Netissima)** : 0,75 % / an
- **Arbitrage** : 0 %
- **Surcoût gestion profilée** : 0,25 % / an (TER des ETF iShares Portfolio — inclus dans la gestion profilée)
- **Frais de rachat** : 0 %
- **Frais de transaction** : 0 %
- **Frais internes des supports** : TER de chaque ETF / fonds, selon le DIC du support concerné
- **Note frais** : 5/5 (source : sinvestir.fr)

## Fonds euros
<!-- Mis à jour : 2026-05-26 -->

- **Fonds disponible** : Netissima (Generali Vie) — garanti à 99,25 % (ou 100 % possible selon conditions)
- **Performance 2024** : 3 % (source : sinvestir.fr — à vérifier sur relevé Generali)
- **Performance 2025** : 3 % (source : sinvestir.fr — à vérifier sur relevé Generali)
- **Performance cumulée 5 ans** : 11,58 % (source : sinvestir.fr)
- **Offre boost 2026-2027** : +1,50 % nets ajoutés au taux de participation aux bénéfices (soit un rendement annoncé de 4,5 % nets en 2026-2027), plafonné à 5,25 % brut de frais de gestion — sous conditions (voir section contraintes)
- **Contraintes d'accès** : 30 % minimum en unités de compte requis pour accéder au taux boosté Netissima ; 100 % fonds euros possible hors boost
- **Note fonds euros** : 4,5/5 (source : sinvestir.fr)

## Unités de compte
<!-- Mis à jour : 2026-05-26 -->

- **Nombre total d'UC** : +700 (gestion libre) / 25 (gestion profilée — ETF iShares Portfolio BlackRock)
- **ETF** : 124 (source : sinvestir.fr — liste complète sur le site Finary)
- **SCPI** : 0 (pas de SCPI disponibles)
- **Actions directes (titres vifs)** : 93 — accessibles uniquement en cours de vie du contrat via versement libre (minimum 1 500 EUR par titre ; non sélectionnables à la souscription initiale)
- **Private equity** : oui
- **Fonds obligataires / OPCVM** : oui (dans les 700+ supports de gestion libre)
- **Rééquilibrage automatique** : oui (en gestion profilée)
- **Note UC** : 3/5 (source : sinvestir.fr)

### ETF Portfolio iShares (gestion profilée)
<!-- Mis à jour : 2026-05-26 -->

Les 3 profils de gestion profilée utilisent exclusivement les ETF iShares Portfolio de BlackRock, composés chacun de 15 à 25 ETF sous-jacents exposant à plus de 8 000 actions et obligations mondiales.

| Profil | Allocation | ETF iShares utilisé | TER ETF |
|---|---|---|---|
| Prudent | 50 % Netissima + 50 % iShares Moderate Portfolio | iShares Moderate Portfolio | 0,25 % |
| Audacieux | 20 % Netissima + 80 % iShares Growth Portfolio | iShares Growth Portfolio | 0,25 % |
| Aventurier | 100 % iShares Growth Portfolio | iShares Growth Portfolio | 0,25 % |

## Conditions et contraintes spécifiques
<!-- Mis à jour : 2026-05-26 -->

- **Versement initial minimum** : 300 EUR (sans versements programmés) / 150 EUR (avec versements programmés mensuels dès 50 EUR / mois)
- **Versements libres minimum** : 300 EUR par opération
- **Versements programmés minimum** : 50 EUR / mois
- **Arbitrage minimum** : Non communiqué [à vérifier]
- **Règles d'arbitrage** : 0 % de frais ; arbitrages effectués via l'espace client Generali (interface Finary directe en cours de développement)
- **Limites d'investissement** :
  - Pas de SCPI disponibles dans le contrat
  - Actions directes : minimum 1 500 EUR par titre, accessibles uniquement via versement libre en cours de vie (non disponibles à la souscription initiale)
  - Bascule entre gestion libre et gestion profilée : non disponible
- **Conditions boost Netissima (jusqu'au 30/06/2026)** :
  - 30 % minimum en UC requis à l'investissement ET au 31 décembre annuel
  - Applicable aux nouveaux contrats et versements libres uniquement (versements programmés, transferts Pacte et rachats Generali exclus)
  - Plafond par versement : 500 000 EUR
  - Rendement plafonné à 5,25 % brut de frais de gestion
  - En cas de retrait, les fonds bénéficiant du boost sont désinvestis en priorité (prorata temporis)
- **Conditions gestion profilée** : 3 profils (Prudent, Audacieux, Aventurier) — TER ETF de 0,25 % / an inclus
- **Transfert entrant** : non disponible (ni depuis Generali, ni depuis un autre assureur)
- **Rachat partiel minimum** : Non communiqué [à vérifier] — procédure par formulaire à adresser à Generali Vie (TSA 70007, 75447 Paris Cedex 09)
- **Rachat total** : clôture définitive du contrat
- **Avance sur contrat** : Non communiqué [à vérifier]
- **Clause bénéficiaire** : standard / libre (modifiable en ligne via l'espace Generali)
- **Garantie financière FGAP** : 70 000 EUR par personne et par assureur (90 000 EUR pour les rentes) — fonds détenus par Generali Vie, distincts des actifs de Finary SAS

### SCPI — Règles spécifiques

Non applicable — aucune SCPI disponible sur ce contrat (source : Finary)

### Fonds euros — Règles spécifiques

- **Accès sans contrainte UC hors boost** : 100 % fonds euros Netissima possible sans obligation d'UC (hors offre boostée)
- **Condition boost 2026-2027** : 30 % minimum en UC requis au moment de l'investissement ET au 31 décembre de chaque année pour bénéficier du taux boosté (+1,50 % nets) — plafond 500 000 EUR par versement
- **Rachat avec boost** : en cas de retrait, les fonds bénéficiant du boost sont désinvestis en priorité (prorata temporis) — source : Finary Help

## Fiscalité
<!-- Mis à jour : 2026-05-26 -->

- **Avant 8 ans** : PFU 30 % (12,8 % IR + 17,2 % PS) ou barème IR + PS
- **Après 8 ans** : abattement annuel 4 600 EUR (célibataire) / 9 200 EUR (couple) puis 24,7 % (7,5 % + 17,2 % PS) ou barème IR + PS
- **Encours > 150 000 EUR après 8 ans** : retour au PFU 30 % sur la fraction excédentaire
- **Exonérations** : licenciement, cessation d'activité par liquidation judiciaire, mise à la retraite anticipée, invalidité 2e ou 3e catégorie
- **Transmission** : abattement 152 500 EUR / bénéficiaire (versements avant 70 ans) ; 30 500 EUR tous bénéficiaires confondus (versements après 70 ans)

## Expérience client
<!-- Mis à jour : 2026-05-26 -->

- **Avis clients** : 3,9/5 (487 avis — source : sinvestir.fr)
- **Note globale plateforme** : 4,7/5 (10 000+ avis toutes fonctionnalités confondues — source : finary.com)
- **App mobile** : oui (intégration directe dans l'app Finary — agrégateur patrimonial)
- **Contact** : email (hello@finary.com), chat en ligne
- **Téléphone** : non
- **Courrier** : non (sauf pour rachats via Generali Vie)
- **Gestion du contrat** : via l'app Finary pour les versements ; via l'espace Generali pour les arbitrages et rachats (interface unifiée Finary en cours de développement)
- **Ergonomie** : très bonne ergonomie et intuitive (source : sinvestir.fr)
- **Services complémentaires** : agrégateur de comptes, exchange crypto (Finary Crypto)
- **Note SAV** : 4,25/5 (source : sinvestir.fr)

## Avis synthétique
<!-- Mis à jour : 2026-05-26 -->

- **Note globale** : 4,35/5 (source : sinvestir.fr)
- **Points forts** :
  - ETF Portfolio iShares de BlackRock exclusifs en France via assurance vie
  - Frais de gestion UC compétitifs : 0,50 % / an
  - 0 % sur versements, arbitrages et rachats
  - App mobile intégrée à l'agrégateur patrimonial Finary (suivi patrimoine unifié)
  - Accès à 700+ supports en gestion libre dont 124 ETF, private equity et 93 actions directes
  - Versement possible dès 150 EUR avec versements programmés (50 EUR / mois)
  - Boost Netissima jusqu'à 4,5 % nets en 2026-2027 (sous conditions)
- **Points faibles** :
  - Pas de SCPI disponibles
  - Actions directes non accessibles à la souscription initiale (minimum 1 500 EUR par titre, en cours de vie uniquement)
  - Bascule entre gestion libre et gestion profilée impossible
  - En gestion profilée, versements programmés uniquement (150 EUR min.) — pas de versements libres ponctuels en gestion profilée
  - Arbitrages gérés via l'espace Generali (interface Finary pas encore unifiée)
  - Transfert entrant non disponible
  - Note clients Trustpilot modérée : 3,9/5 (487 avis)
- **Services complémentaires** : agrégateur patrimonial (comptes bancaires, investissements, immobilier, crypto), Finary Crypto, Finary One (gestion privée)

## Sources

- [Finary — Page Assurance Vie Finary Life](https://www.finary.com/fr/invest/assurance-vie)
- [Finary — Liste des supports UC](https://finary.com/fr/invest/assurance-vie/finary-life-liste-supports-uc)
- [Finary Help — Quels sont les frais de Finary Life ?](https://help.finary.com/fr/articles/10495134)
- [Finary Help — Montant minimum pour ouvrir un contrat](https://help.finary.com/fr/articles/10494300)
- [Finary Help — Effectuer un versement libre](https://help.finary.com/fr/articles/13729839)
- [Finary Help — Les trois profils investisseurs](https://help.finary.com/fr/articles/10495292)
- [Finary Help — Conditions du boost Netissima](https://help.finary.com/fr/articles/10495386)
- [Finary Help — Supports disponibles en gestion libre](https://help.finary.com/fr/articles/15062711)
- [Finary Help — Rachat total ou partiel](https://help.finary.com/fr/articles/11521011)
- [Finary Help — Transfert vers Finary Life](https://help.finary.com/fr/articles/10494342)
- [Finary Help — Sécurité des fonds](https://help.finary.com/fr/articles/10495150)
- [Finary Help — Fiscalité de l'assurance vie](https://help.finary.com/fr/articles/10495142)
- [S'investir — Comparatif Assurance Vie](https://simulateurs.sinvestir.fr/les-comparateurs/assurance-vie)
